...
- 请假流程已形成正式业务闭环：员工申请、重复申请拦截、部门假期额度校验、人事审批/驳回、员工撤销与历史删除均真实持久化。
- 请假审批通过后，系统会按开始日期和请假天数自动把对应考勤日期更新为“请假”，日期统一按 `yyyy-MM-dd` 传输，避免时区偏移。
- 部门、城市和角色增加数据完整性保护：存在子部门、在职员工、社保引用或员工角色引用时禁止删除；安全删除时自动清理部门规则和角色菜单关系。
- 部门名称、城市名称、角色名称和角色编号由服务端执行唯一性校验，社保缴费比例限制在 `0` 到 `1` 之间。
- 关闭动态路由的全量预取，首屏只加载登录所需核心资源；业务页面在用户访问时按需加载，正式接口超时由 `10` 秒提高至 `30` 秒以改善弱网环境稳定性。
