...
public class HomeService {
    ...
    public ResponseDTO getStaffData() {
        Timestamp yearStart = Timestamp.valueOf(DateUtil.thisYear() + "-01-01 00:00:00");
        QueryWrapper<Staff> queryWrapper = new QueryWrapper<Staff>().ge("create_time", yearStart);
    ...
    public ResponseDTO getCountData() {
        ...
        long lateNum = this.attendanceService.count(new QueryWrapper<Attendance>().eq("attendance_date", today).eq("status", AttendanceStatusEnum.LATE.getCode()));
        long leaveEarlyNum = this.attendanceService.count(new QueryWrapper<Attendance>().eq("attendance_date", today).eq("status", AttendanceStatusEnum.LEAVE_EARLY.getCode()));
        long absenteeismNum = this.attendanceService.count(new QueryWrapper<Attendance>().eq("attendance_date", today).eq("status", AttendanceStatusEnum.ABSENTEEISM.getCode()));
